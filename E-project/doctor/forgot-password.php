...
function sendRecoveryLink($d_token, $d_email, $d_name) {
    ...
    try {
        ...
        $mail->addAddress($d_email, $d_name);     // Add a recipient
        // Content
        $mail->isHTML(true);                                  // Set email format to HTML
        $mail->Subject = 'PASSWORD RECOVERY REQUEST';
        $mail->Body    = 'A request has been generated to change your password. Please <b><a href="http://localhost:82/care/doctor/resetpass.php?d_token='.$d_token.'&d_email='.$d_email.'">Click here</a></b> to reset your password.';
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                Swal.fire({
                    title: "Email sent",
                    text: "Email has been sent to '.$d_email.'. Please check your inbox.",
                    icon: "success"
                });
            });
        </script>';
...
if(isset($_POST['sendlink']) && $_SERVER['REQUEST_METHOD']=="POST"){
    $d_email = mysqli_real_escape_string($connection, $_POST['recoveryemail']);
    $d_token = md5(rand());
    ...
    if($result){
        if(mysqli_num_rows($result) > 0){
            ...
            $updatetoken = "UPDATE `doctors` SET `d_token`='$d_token' WHERE d_email='$d_email'; ";
            $updatetoken_run = mysqli_query($connection, $updatetoken) or die("failed to execute");
            
            if($updatetoken_run){
                sendRecoveryLink($d_token, $d_email, $d_name);
            }
        }
        else{
             echo '
